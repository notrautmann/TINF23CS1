...
def get_products(products_id):
    ...
    """
    result = read(Products, id=products_id)
    if result is None:
...
def create_products():
    ...
    Return:
        json-structure: Returns status code and if operation succeeded the returned data
            otherwise an error message
    """
    product_obj = Products(name=request.values.get('name'),
...
def update_products(products_id):
    ...
    result = update(Products, products_id, **changes)
    if result is None:
        return jsonify({'success': False, 'error': 'error when writing data'}), 500
    return jsonify({'success': True, 'data': result}), 200
...
def delete_products(products_id):
    ...
    Return:
        json-structure: Returns status code and if operation succeeded the returned data
            otherwise an error message
    """
    result = delete(Products, products_id)
    if result is None:
        return jsonify({'success': False, 'error': 'error when writing data'}), 500
    return jsonify({'success': True, 'data': result}), 200
