...
def get_supplier_invoices(supplier_invoices_id):
    ...
    """
    result = read(Supplier_invoices, id=supplier_invoices_id)
    if result is None:
...
def create_supplier_invoices():
    ...
    Return:
        json-structure: Returns status code and if operation succeeded the returned data
            otherwise an error message
    """
    obj = Supplier_invoices(invoice_number=request.values.get('invoice_number'),
...
def update_supplier_invoices(supplier_invoices_id):
    ...
    result = update(Supplier_invoices, supplier_invoices_id, **changes)
    if result is None:
        return jsonify({'success': False, 'error': 'error when writing data'}), 500
    return jsonify({'success': True, 'data': result}), 200
...
def delete_supplier_invoices(supplier_invoices_id):
    ...
    Return:
        json-structure: Returns status code and if operation succeeded the returned data
            otherwise an error message
    """
    result = delete(Supplier_invoices, supplier_invoices_id)
    if result is None:
        return jsonify({'success': False, 'error': 'error when writing data'}), 500
    return jsonify({'success': True, 'data': result}), 200
