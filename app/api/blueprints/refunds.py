"""
Implements the CRUD-operations for the refunds-table.

Functions:

    get_refunds(refunds_id)
    create_refunds()
    update_refunds(refunds_id)
    delete_refunds(refunds_id)
...
def get_refunds(refunds_id):
    ...
    """
    result = read(Refunds, id=refunds_id)
    if result is None:
...
def create_refunds():
    ...
    Return:
        json-structure: Returns status code and if operation succeeded the returned data
            otherwise an error message
    """
    refund_obj = Refunds(receipt_id=request.values.get('receipt_id'),
...
def update_refunds(refunds_id):
    ...
    result = update(Refunds, refunds_id, **changes)
    if result is None:
        return jsonify({'success': False, 'error': 'error when writing data'}), 500
    return jsonify({'success': True, 'data': result}), 200
...
def delete_refunds(refunds_id):
    ...
    Return:
        json-structure: Returns status code and if operation succeeded the returned data
            otherwise an error message
    """
    result = delete(Refunds, refunds_id)
    if result is None:
        return jsonify({'success': False, 'error': 'error when writing data'}), 500
    return jsonify({'success': True, 'data': result}), 200
