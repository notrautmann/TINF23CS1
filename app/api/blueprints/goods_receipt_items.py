"""
Implements the CRUD-operations for the goods_receipt_items-table.

Functions:

    get_goods_receipt_items(goods_receipt_items_id)
    create_goods_receipt_items()
    update_goods_receipt_items(goods_receipt_items_id)
    delete_goods_receipt_items(goods_receipt_items_id)
...
goods_receipt_items_bp = Blueprint('goods_receipt_items',
    __name__,
    url_prefix='/goods_receipt_items')
...
def get_goods_receipt_items(goods_receipt_items_id):
    ...
    """
    result = read(Goods_receipt_items, id=goods_receipt_items_id)
    if result is None:
...
def create_goods_receipt_items():
    ...
    Return:
        json-structure: Returns status code and if operation succeeded the returned data
            otherwise an error message
    """
    obj = Goods_receipt_items(receipt_id=request.values.get('receipt_id'),
...
def update_goods_receipt_items(goods_receipt_items_id):
    ...
    result = update(Goods_receipt_items, goods_receipt_items_id, **changes)
    if result is None:
        return jsonify({'success': False, 'error': 'error when writing data'}), 500
    return jsonify({'success': True, 'data': result}), 200
...
def delete_goods_receipt_items(goods_receipt_items_id):
    ...
    Return:
        json-structure: Returns status code and if operation succeeded the returned data
            otherwise an error message
    """
    result = delete(Goods_receipt_items, goods_receipt_items_id)
    if result is None:
        return jsonify({'success': False, 'error': 'error when writing data'}), 500
    return jsonify({'success': True, 'data': result}), 200
