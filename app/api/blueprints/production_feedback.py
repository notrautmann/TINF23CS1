...
def get_production_feedback(production_feedback_id):
    ...
    """
    result = read(Production_feedback, id=production_feedback_id)
    if result is None:
...
def create_production_feedback():
    ...
    Return:
        json-structure: Returns status code and if operation succeeded the returned data
            otherwise an error message
    """
    feedback_obj = Production_feedback(plan_item_id=request.values.get('plan_item_id'),
...
def update_production_feedback(production_feedback_id):
    ...
    result = update(Production_feedback, production_feedback_id, **changes)
    if result is None:
        return jsonify({'success': False, 'error': 'error when writing data'}), 500
    return jsonify({'success': True, 'data': result}), 200
...
def delete_production_feedback(production_feedback_id):
    ...
    Return:
        json-structure: Returns status code and if operation succeeded the returned data
            otherwise an error message
    """
    result = delete(Production_feedback, production_feedback_id)
    if result is None:
        return jsonify({'success': False, 'error': 'error when writing data'}), 500
    return jsonify({'success': True, 'data': result}), 200
