...
def get_employees(employees_id):
    ...
    """
    result = read(Employees, id=employees_id)
    if result is None:
...
def create_employees():
    ...
    Return:
        json-structure: Returns status code and if operation succeeded the returned data
            otherwise an error message
    """
    employee_obj = Employees(first_name=request.values.get('first_name'),
...
def update_employees(employees_id):
    ...
    result = update(Employees, employees_id, **changes)
    if result is None:
        return jsonify({'success': False, 'error': 'error when writing data'}), 500
    return jsonify({'success': True, 'data': result}), 200
...
def delete_employees(employees_id):
    ...
    Return:
        json-structure: Returns status code and if operation succeeded the returned data
            otherwise an error message
    """
    result = delete(Employees, employees_id)
    if result is None:
        return jsonify({'success': False, 'error': 'error when writing data'}), 500
    return jsonify({'success': True, 'data': result}), 200
